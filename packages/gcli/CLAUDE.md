...
- `pickProviderInteractive()` — production `deps.pickProvider`: arrow-key menu rendered entirely on **stderr** (stdout stays pipe-clean) with in-place ANSI redraw (cursor-up + `\r` + clear-to-EOL per line). Zero deps: `readline.emitKeypressEvents` + stdin raw mode, restored on EVERY exit path before any backend spawn. Visual frame (2026-09 重设计): 2-line title (cyan bold `◆ gcli` + dim keymap hint) → dim separator → aligned entries (name column = max+2, missing quota → dim `—`) → dim `Esc 不切换` footer; selected row = sage `❯` + bold name + truecolor bg `#292e42`; quota windows colored per-window by `levelColor` (statusline-sage thresholds 85/60, truecolor sage/amber/vermillion), `↻rel` dim, remembered row tagged ` ●上次` (cyan, via `PickerEntry.tag`). `NO_COLOR` non-empty → zero ANSI. Row construction lives in the exported pure `renderPickerRows(entries, selectedIndex, noColor)`; the old selectable 不切换 row is gone (Esc is the skip path).
