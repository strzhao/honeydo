...
The `api` subcommand is the third path: a zero-subprocess, direct anthropic-compatible `/v1/messages` HTTP call (fetch + SSE aggregation, no agent). Use it when a script needs one-shot LLM text without spawning a CLI agent; full flag reference lives in `gcli api --help`. Two gotchas it guards against (2026-09 flaky-handoff investigation): bigmodel GLM endpoints default thinking ON and share the `max_tokens` budget with text, so small budgets can yield thinking-only responses — `--thinking off|on` controls the field explicitly (`auto` = omit, k3 quality default) and "no text content" errors always name the response's block types + stop_reason; transient failures (network/408/429/5xx/empty body) auto-retry via `--retry N` (default 1, exponential backoff), with each attempt noted on stderr — deterministic failures are never retried.
...
- `pickProviderInteractive()` — production `deps.pickProvider`: arrow-key menu rendered entirely on **stderr** (stdout stays pipe-clean) with in-place ANSI redraw (cursor-up + `\r` + clear-to-EOL per line). Zero deps: `readline.emitKeypressEvents` + stdin raw mode, restored on EVERY exit path before any backend spawn. Visual frame (2026-09 重设计): 2-line title (cyan bold `◆ gcli` + dim keymap hint) → dim separator → aligned entries (name column = max+2, missing quota → dim `—`) → dim `Esc 退出` footer; selected row = sage `❯` + bold name + truecolor bg `#292e42`; quota windows colored per-window by `levelColor` (statusline-sage thresholds 85/60, truecolor sage/amber/vermillion), `↻rel` dim, remembered row tagged ` ●上次` (cyan, via `PickerEntry.tag`). `NO_COLOR` non-empty → zero ANSI. Row construction lives in the exported pure `renderPickerRows(entries, selectedIndex, noColor)`; the old selectable 不切换 row is gone — **Esc/C-g skip = 退出 gcli（exit 0 + stderr 提示，不 spawn claude）**; isolated-ESC byte fast path (50ms, readline has no escape disambiguation timeout without an interface) because real terminals' lone `\x1b` would otherwise never emit a keypress.
...
- `spawnInteractive()` / `runAgyInteractive()` / `runClaudeInteractive()` — interactive TUI spawn: fully inherited stdio, no timeout; child exit code passed through (signal → 128+signo).
...
- **Provider picker** (claude path incl. default, TTY, no `--provider`): arrow-key menu (cc-switch DB order) on stderr with quota subtitles (kimi/glm live usage `5h:P% ↻<rel> wk:P% ↻<rel>` — per-window reset, cached 60s); ↑↓/j/k/C-n/C-p move, Enter confirms, Esc/C-g exits gcli (exit 0, no backend spawn), M-</M-> jump, ctrl-c exits 130. The confirmed choice is remembered in `~/.config/gcli/last-provider`; print mode silently reuses it (one stderr hint, `--pick` forces the menu). Lookup failures degrade to a soft stderr warning without changing the exit code. Non-TTY callers (skills/CI/pipes) never trigger it — zero prompts, zero cc-switch DB reads, zero memory-file IO, zero quota requests, byte-identical behaviour.
